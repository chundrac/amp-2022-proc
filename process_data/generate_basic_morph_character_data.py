...
cogs_to_keep = [k for k in cogvar.keys() if len(cogvar[k])==4 and cogcount[k] > L/10]

acd_data = [l for l in acd_data if l[0] in cogs_to_keep]

cog_values = {}
for l in acd_data:
    if l[0] not in cog_values.keys():
        cog_values[l[0]] = defaultdict(list)
    if l[-1] == 'basic':
        b = 'TRUE'
    else:
        b = 'FALSE'
    cog_values[l[0]][l[1]].append((b,l[12]))
...
f = open('../data/basic_morph_character_data.tsv','w')
